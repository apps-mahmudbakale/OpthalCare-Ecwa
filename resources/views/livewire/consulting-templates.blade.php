<div>
                        ...
                        <td>
                            <div class="d-inline-block"><a href="javascript:;" class="dropdown hide-arrow"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end m-0">
                                    <li><a href=""
                                            wire:click.prevent="selectConsultingTemplate({{ $template->id }})"
                                            class="dropdown-item">Edit</a></li>
                                    <div class="dropdown-divider"></div>
                                    <li><a id="temp_delete{{ $template->id }}" data-value="{{ $template->id }}"
                                            class="dropdown-item text-danger delete-record">Delete</a></li>
                                </ul>
                            </div>
                            <script>
                                document.querySelector('#temp_delete{{ $template->id }}').addEventListener('click', function(e) {
                                    ...
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            document.getElementById('temp_delete#' + this.getAttribute('data-value')).submit();
                                        }
                                    })
                                })
                            </script>
                            <form id="temp_delete#{{ $template->id }}"
                                action="{{ route('app.appointment-type.destroy', $template->id) }}" method="POST"
    ...
    <script>
        window.addEventListener('ConsultingTemplateEditModal', function() {
            $('#edit-consulting-template-modal').modal('show');
        });
    </script>
    @include('_partials._modals.modal-new-consulting-template')
    @include('_partials._modals.modal-edit-consulting-template')
</div>
