<div>
                        ...
                        <td>
                            <div class="d-inline-block"><a href="javascript:;" class="dropdown hide-arrow"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end m-0">
                                    <li><a href=""
                                            wire:click.prevent="selectConsultingRoom({{ $consultingRoom->id }})"
                                            class="dropdown-item">Edit</a></li>
                                    <div class="dropdown-divider"></div>
                                    <li><a id="dele{{ $consultingRoom->id }}" data-value="{{ $consultingRoom->id }}"
                                            class="dropdown-item text-danger delete-record">Delete</a></li>
                                </ul>
                            </div>
                            <script>
                                document.querySelector('#dele{{ $consultingRoom->id }}').addEventListener('click', function(e) {
                            ...
                            <form id="dele#{{ $consultingRoom->id }}"
                                action="{{ route('app.consulting-rooms.destroy', $consultingRoom->id) }}" method="POST"
        ...
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    Showing {{ $consultingRooms->firstItem() }} to {{ $consultingRooms->lastItem() }} of
                    {{ $consultingRooms->total() }} entries</div>
            </div>
            <div class="col-sm-12 col-md-6">
    ...
    @include('_partials._modals.modal-edit-consulting-room')
</div>
