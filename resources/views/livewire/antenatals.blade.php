<div>
                        ...
                        <td>
                            <div class="d-inline-block"><a href="javascript:;" class="dropdown hide-arrow"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end m-0">
                                    <li><a href="" wire:click.prevent="selectAntenatal({{ $antenatal->id }})"
                                            class="dropdown-item">Edit</a></li>
                                    <div class="dropdown-divider"></div>
                                    <li><a id="delet{{ $antenatal->id }}" data-value="{{ $antenatal->id }}"
                                            class="dropdown-item text-danger delete-record">Delete</a></li>
                                </ul>
                            </div>
                            <script>
                                document.querySelector('#delet{{ $antenatal->id }}').addEventListener('click', function(e) {
                            ...
                            <form id="delet#{{ $antenatal->id }}"
                                action="{{ route('app.antenatals.destroy', $antenatal->id) }}" method="POST"
        ...
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    Showing {{ $antenatals->firstItem() }} to {{ $antenatals->lastItem() }} of
                    {{ $antenatals->total() }} entries</div>
            </div>
            <div class="col-sm-12 col-md-6">
    ...
    <script>
        window.addEventListener('AntenatalEditModal', function() {
            $('#edit-antenatal-modal').modal('show');
        });
    </script>
    @include('_partials._modals.modal-new-antenatal')
    @include('_partials._modals.modal-edit-antenatal')
</div>
