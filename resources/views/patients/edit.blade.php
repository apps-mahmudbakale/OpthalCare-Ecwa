      ...
      <div class="card-body">
        <form action="{{ route('app.patients.update', $patient->id) }}" method="POST">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-12">
              <label class="form-label" for="tag_ids">Patient Tags</label>
              <select name="tag_ids[]" id="tag_ids" class="form-control select2" multiple>
                @foreach(\App\Models\Tag::all() as $tag)
                <option value="{{ $tag->id }}"
                        {{ in_array($tag->id, $patient->assignedTags->pluck('tag_id')->toArray()) ? 'selected' : '' }}>
                  {{ $tag->name }}
                </option>
                @endforeach
              </select>
            ...
            <div class="col-md-12">
              ...
              <input name="next_of_kin_address" value="{{ $patient->user->nok->next_of_kin_address ?? '' }}"
            ...
            <div class="col-md-4">
              ...
              <select name="hmo_plan_id" id="hmo_plan" class="form-control">
                ...
                @foreach ($hmos as $hmo)
                <option value="{{ $hmo->id }}" {{ ($patient->hmo_plan_id == $hmo->id) ? 'selected' : '' }}>{{ $hmo->name }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-4">
