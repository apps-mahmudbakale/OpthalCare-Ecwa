    ...
    <div class="card">
        <div class="px-3 py-4">
            ...
            <form action="{{ route('app.cashpoints.bill-patient') }}" method="post">
                ...
                <div class="form-group">
                    <div class="form-label-group">
                        <input type="text" id="patient-search" readonly disabled
                            value="{{ ucfirst($tempPatient->first_name) }} {{ ucfirst($tempPatient->last_name) }}"
                            class="form-control" placeholder="Search for patients...">
                    </div>
                </div>
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $('#service-category').change(function() {
                        ...
                        $.ajax({
                            url: '{{ route('bill.services') }}', // Replace with your actual API endpoint
                            type: 'POST',
                            ...
                            success: function(data) {
                                ...
                                if (data.error) {
                                    $('#service_id').append(
                                        `<option disabled>Error: ${data.error}</option>`
                                    ...
                                    data.forEach(item => {
                                        ...
                                        if (item.ward) {
                                            $('#service_id').append(
                                                `<option value="${item.id}">${item.name} - ${item.ward.name}</option>`
                                                );
                                        } else {
                                            $('#service_id').append(
